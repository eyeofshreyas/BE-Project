...
| Verified | Translated a real fine-tuned-model summary to Hindi — grammatically correct, meaning preserved:<br>*"अपीलार्थी को धारा 305, आई. पी. सी. के तहत दोषी ठहराया गया और मौत की सजा सुनाई गई। उच्च न्यायालय ने दोषसिद्धि और सजा की पुष्टि की।..."* |
| Number corruption (fixed) | The "minor numeral slip" first noted here turned out to be systematic, not minor. Measured on *"convicted under Section **302**"*, the model returned **303** in 4 of 6 target languages — and identically so on the old `-dist-200M`, so it is an IndicTrans2-family flaw, not something the InLegalTrans swap introduced. IPC 302 and 303 are different offences, so this is a correctness bug, not a cosmetic one. Fixed in `translate.py:restore_numbers()`, which pastes the source's numbers back over the translation positionally (dates and case numbers too) and raises if the counts differ rather than guessing. Now 6/6 languages correct; asserts run via `python3 translate.py --self-check`. |
