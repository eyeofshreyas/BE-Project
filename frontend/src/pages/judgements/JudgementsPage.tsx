    ...
    <div className={styles.page}>
      <div className={styles.wrap}>
        <div className={styles.header}>
          ...
          <div className={styles.primaryChip} onClick={openForm}><Icon name="plus" size={15} color="#FCFAF4" /> Add Judgement</div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <input
            ...
            style={{ flex: 1, minWidth: 240, padding: '9px 14px', borderRadius: 3, border: '1px solid #CFC6B0', fontSize: 13.5, background: '#FCFAF4' }}
          />
          <div style={{ display: 'flex', gap: 4, background: '#E6E0CE', borderRadius: 3, padding: 4, flexWrap: 'wrap' }}>
            {(['All', ...OUTCOMES] as const).map((f) => {
              ...
              return (
                <div
                  ...
                  style={{ padding: '7px 12px', borderRadius: 3, fontSize: 12.5, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap', color: outcomeFilter === f ? '#1A1A17' : '#575145', background: outcomeFilter === f ? '#FCFAF4' : 'transparent' }}
                >
                  {f} ({count})
                </div>
              )
                ...
                <div key={j.id} onClick={() => navigate(`/judgements/${j.id}`)} className={styles.panelCard} style={{ display: 'flex', flexDirection: 'column', gap: 12, cursor: 'pointer' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                    <div style={{ display: 'flex', gap: 12 }}>
                      ...
                      <div>
                        <div style={{ fontSize: 15, fontWeight: 700, color: '#1A1A17' }}>{j.case_title ?? j.case_number ?? 'Untitled matter'}</div>
                  ...
                  <div style={{ fontSize: 13.5, color: '#33302A' }}>{j.summary}</div>

                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 20, borderTop: '1px solid #F1EDE0', paddingTop: 12 }}>
                    <div>
                      <div style={{ fontSize: 9.5, fontWeight: 700, color: MUTED, fontFamily: "'IBM Plex Mono',monospace", textTransform: 'uppercase', letterSpacing: '.13em' }}>Bench</div>
                      <div style={{ fontSize: 13, color: '#1A1A17', marginTop: 2 }}>{j.bench}</div>
                    </div>
                    <div>
                      <div style={{ fontSize: 9.5, fontWeight: 700, color: MUTED, fontFamily: "'IBM Plex Mono',monospace", textTransform: 'uppercase', letterSpacing: '.13em' }}>Relief</div>
                      <div style={{ fontSize: 13, color: '#1A1A17', marginTop: 2 }}>
                        {j.relief_amount != null ? money(j.relief_amount) : ''}{j.relief_amount != null && j.relief_text ? ' ' : ''}{j.relief_text ?? (j.relief_amount == null ? '—' : '')}
                      </div>
                    </div>
                    <div>
                      <div style={{ fontSize: 9.5, fontWeight: 700, color: MUTED, fontFamily: "'IBM Plex Mono',monospace", textTransform: 'uppercase', letterSpacing: '.13em' }}>Case</div>
                      <div className={styles.tdMono} style={{ fontSize: 13, marginTop: 2 }}>{j.case_number ?? '—'}</div>
                    </div>
                    <div>
                      <div style={{ fontSize: 9.5, fontWeight: 700, color: MUTED, fontFamily: "'IBM Plex Mono',monospace", textTransform: 'uppercase', letterSpacing: '.13em' }}>Appeal</div>
                      <div style={{ fontSize: 13, color: '#1A1A17', marginTop: 2 }}>{j.appeal_status ?? '—'}</div>
                    </div>
        ...
        {formOpen && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(35, 48, 107,.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: 20 }} onClick={() => setFormOpen(false)}>
            <div style={{ background: '#FCFAF4', borderRadius: 3, padding: 24, width: 460, maxHeight: '86vh', overflowY: 'auto', boxShadow: '0 20px 48px rgba(0,0,0,.2)' }} onClick={(e) => e.stopPropagation()}>
              <div style={{ fontFamily: "'Spectral', serif", fontSize: 16, fontWeight: 700, color: '#1A1A17', marginBottom: 16 }}>Add Judgement</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                {formError && <div style={{ fontSize: 12.5, color: '#B3282D' }}>{formError}</div>}
...
const inputStyle: React.CSSProperties = { width: '100%', boxSizing: 'border-box', padding: '9px 12px', borderRadius: 3, border: '1.5px solid #CFC6B0', fontSize: 13.5, background: '#FCFAF4', fontFamily: 'inherit' }
